...
int find_process_by_name(const char *process_name)
{
	...
	char temp_process_name[PATH_MAX + 1], temp_command[PATH_MAX + 1];
	strncpy(temp_process_name, process_name, PATH_MAX + 1);
	process_name = basename(temp_process_name);
	init_process_iterator(&it, &filter);
	while (get_next_process(&it, &proc) != -1)
	{
		strncpy(temp_command, proc.command, PATH_MAX + 1);
		// process found
		if (strcmp(basename(temp_command), process_name) == 0)
		{
